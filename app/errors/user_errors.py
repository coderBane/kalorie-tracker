...
def email_exists(email_address: str) -> Error:
    return Error.conflict(DUPLICATE, f"Email Address: ({email_address}) already in use.")
...
def not_found_by_email(email_address: str) -> Error:
    return Error.not_found(NOTFOUND, f"User (Email Address: {email_address}) does not exist.")
